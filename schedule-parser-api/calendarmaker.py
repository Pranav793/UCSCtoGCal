...
def parse_days_times(days_times_str):
    ...
    # Parse the time range, e.g. "4:00PM - 5:05PM"
    tm = re.match(r'(.*)\s*-\s*(.*)', time_part)
    if not tm:
        return ics_days, None, None
    start_str, end_str = tm.groups()
...
def create_events_for_course(course):
    """
    Convert each local time to real UTC, so e.g. Monday 16:00 PST => 
    20250107T000000Z. Google then displays it as Monday 16:00 PST, 
    preventing Sunday shift.
    """
    events = []
    course_title = course.get("title", "Untitled Course")

    # We'll define local zone for PST/PDT
    la_zone = ZoneInfo("America/Los_Angeles")
    # Also define UTC zone
...
def build_calendar(courses):
    """
    Build a calendar with each event in real UTC. 
    Google sees them, displays correct local times, no Sunday shift.
    """
    cal = Calendar()
    for course in courses:
        evts = create_events_for_course(course)
