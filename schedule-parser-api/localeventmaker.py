...
# For ordering if you have multiple days in e.g. "MoWeFr":
ICS_DAY_ORDER = ["MO","TU","WE","TH","FR","SA","SU"]

def parse_days_times(days_times_str):
    """
    e.g. "MoWeFr 4:00PM - 5:05PM" -> (["MO", "WE", "FR"], datetime.time(16,0), datetime.time(17,5))
    """
    # Split the day codes (MoWeFr) from the time range
    m = re.match(r'^([A-Za-z]+)\s+(.*)$', days_times_str.strip())
    if not m:
        return [], None, None
    days_part, time_part = m.groups()
...
def parse_time_12h(tstr):
    ...
    """
    tstr = tstr.strip()
    # Insert space before AM/PM => "4:00PM" -> "4:00 PM"
    if re.match(r'^\d{1,2}:\d{2}(AM|PM)$', tstr):
        tstr = tstr[:-2] + " " + tstr[-2:]
        dt = datetime.strptime(tstr, "%I:%M %p")
        return dt.time()
...
def align_first_occurrence(start_date, wday_ics):
    """
    e.g. start_date=2025-01-06 (Mon), wday_ics="WE" => shift +2 days => 2025-01-08
    """
    PY_DAY_MAP = {"MO":0, "TU":1, "WE":2, "TH":3, "FR":4, "SA":5, "SU":6}
    target_wd  = PY_DAY_MAP[wday_ics]
    current_wd = start_date.weekday()  # Monday=0..Sunday=6
...
def create_events_for_course(course):
    """
    Creates ICS events in *true local time* (America/Los_Angeles), which ics will store in UTC.
    => No "8 hours early" problem, since 4PM PST becomes 00:00 next day UTC, 
       and your calendar reverts it back to 4PM PST.
    ...
    for cls_info in course["classes"]:
        ...
        # Align to earliest day in weekdays
        earliest_wd = min(weekdays, key=lambda d: ICS_DAY_ORDER.index(d))
        first_occ   = align_first_occurrence(start_date, earliest_wd)

        # Build a single ICS event
        e = Event()
        e.name = f"{course_title} ({comp} {sect})"
        e.description = f"Instructor: {instr}\nClass Number: {cnum}"
        e.location = room

        # Create local aware datetimes => store them with tz=America/Los_Angeles
        la_tz = ZoneInfo("America/Los_Angeles")
        start_dt = datetime.combine(first_occ, start_t, la_tz)
        end_dt   = datetime.combine(first_occ, end_t,   la_tz)
        e.begin  = start_dt
        e.end    = end_dt
