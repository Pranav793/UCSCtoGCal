...
def create_events_for_course(course):
    """
    Hardcodes "America/Los_Angeles" in ICS lines so we don't get "Sunday" shifts.
    We'll build:
      DTSTART;TZID=America/Los_Angeles:YYYYmmddTHHMMSS
      DTEND;TZID=America/Los_Angeles:YYYYmmddTHHMMSS
      RRULE:FREQ=WEEKLY;BYDAY=MO,WE,FR;...
    and skip e.begin/e.end entirely.
    """
    events = []
    course_title = course.get("title", "Untitled Course")
    ...
    for cls_info in course["classes"]:
        ...
        # Align to earliest day
        earliest_wd = min(weekdays, key=lambda d: ICS_DAY_ORDER.index(d))
        first_occ   = align_first_occurrence(start_date, earliest_wd)
        
        # Build naive datetimes for that earliest day at start/end times
        start_dt = datetime.combine(first_occ, start_t)
        end_dt   = datetime.combine(first_occ, end_t)

        # Convert them to ICS format, e.g. "20250106T160000"
        start_ics = start_dt.strftime("%Y%m%dT%H%M%S")
        end_ics   = end_dt.strftime("%Y%m%dT%H%M%S")
        
        # We'll create an Event but NOT set e.begin/e.end:
        e = Event()
        e.name        = f"{course_title} ({comp} {sect})"
        e.description = f"Instructor: {instr}\nClass Number: {cnum}"
        e.location    = room

        # Hardcode the local time zone in ICS lines:
        # e.g. "DTSTART;TZID=America/Los_Angeles:20250106T160000"
        e.extra.append(
            ContentLine(
        ...
        # Add a single RRULE with multi-day BYDAY
        until_utc = end_date.strftime("%Y%m%dT235900Z")
        e.extra.append(
            ContentLine(
                name="RRULE",
        ...
        # We skip e.make_all_day(False), e.begin, e.end, etc.
        # The lines above override them with a local time zone approach.
